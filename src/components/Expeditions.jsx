...
export default function Expeditions({ onOpenBooking }) {
  return (
    <section id="expeditions" className="relative py-28 px-6 sm:px-10 md:px-16 bg-void-950">
      {/* Ambience glow */}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-20">
          <span className="font-mono text-xs tracking-widest text-ember uppercase bg-ember/10 border border-ember/30 px-3 py-1 rounded-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {tiers.map((tier) => (
            <div
              key={tier.name}
              className={`rounded-3rem p-8 sm:p-10 flex flex-col justify-between relative transition-all duration-300 border ${
                tier.theme
              } ${tier.popular ? 'md:-translate-y-4 ring-2 ring-ember/60' : 'hover:border-white/25'}`}
            >
              {/* Popular Badge */}
              {tier.popular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-ember to-plasma text-white text-[11px] font-mono tracking-wider font-bold px-4 py-1 rounded-full shadow-lg flex items-center gap-1.5">
              ...
              <div>
                {!tier.popular && (
                  <span className="font-mono text-[11px] text-plasma font-semibold tracking-wider block mb-2">
                    {tier.tag}
                  </span>
                )}
                ...
                <div className="mb-6 pt-4 border-t border-white/10">
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-3xl sm:text-4xl font-sans font-bold text-white">
                      {tier.price}
                    </span>
                  ...
                  <div className="text-[11px] font-mono text-cyanGlow mt-1">
              ...
              <button
                ...
                className={`btn-magnetic w-full py-3.5 px-6 rounded-full font-semibold text-sm transition-all flex items-center justify-center gap-2 ${
                  tier.popular
